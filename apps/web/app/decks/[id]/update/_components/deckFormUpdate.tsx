...
export const DeckFormUpdate: React.FC<DeckFormProps> = ({ formats }) => {
  ...
  const [updatedCard, setUpdatedCard] = useState<Partial<DeckCards> | null>(null);
  const { id } = useParams();
  ...
  const addCard = (card: DeckCards) => {
    ...
    const existingFormCard = currentCards.find(c => c.cardId === card.cardId && c.role === card.role);
    const deckCard = deck?.cards?.find(c => c.card.id === card.cardId);
    if (deckCard && deckCard.role === card.role) {
      setCardsToUpdate(prev => {
        const existingUpdate = prev.find(c => c.cardId === deckCard.cardId && c.role === card.role);
        if (existingUpdate) {
          ...
          return [...prev, { id: deckCard.id, qty: deckCard.qty + card.qty, role: card.role }];
        }
      });
      setCardsMap(prev => {
        const existing = prev.find((c : any) => c.id === deckCard.id && c.role === card.role);
        if (existing) {
          return prev.map(c =>
              c.id === deckCard.id && c.role === card.role ? { ...c, qty: c.qty + card.qty } : c
          );
        } else {
          return [...prev, { cardId: card.cardId, id: deckCard.id , name: card.name, qty: deckCard.qty + card.qty, role: card.role }];
        }
      });
    }
    else if (existingFormCard)
    {
      form.setValue(
          "cards",
          currentCards.map(c =>
              c.cardId === card.cardId ? { ...c, qty: c.qty + card.qty, role: card.role } : c
          )
      );
      setCardsMap(prev => {
        const existing = prev.find(c => c.cardId === card.cardId && c.role === card.role);
        if (existing){
          return prev.map(c =>
          ...
          return [...prev, { cardId: card.cardId, name: card.name, qty: card.qty, role: card.role }];
        }
      });
    }
    else {
      form.setValue("cards", [...currentCards, { cardId: card.cardId, qty: card.qty, role: card.role }]);
      setCardsMap(prev => [...prev, { cardId: card.cardId, name: card.name, qty: card.qty, role: card.role }]);
    }
  };

  const updateCard = (card: Partial<DeckCards>, oldCard: Pick<DeckCards, "qty" | "role">) => {
    const currentCards = form.getValues("cards") || [];
    const existingFormCard = currentCards.find(c => c.cardId === card.cardId && c.role === oldCard.role);
    if (card?.id) {
      ...
      if (deckCard)
      {
        ...
        setCardsMap(prev => {
          ...
          if (existing) {
            ...
            return [...prev, { cardId: card.cardId, id: deckCard.id , name: card.name, qty: card.qty, role: card.role }];
          }
        });
      }
    }
    else if (existingFormCard)
    {
      ...
      setCardsMap(prev => {
        const existing = prev.find(c => c.cardId === card.cardId && c.role === oldCard.role);
  ...
  const removeCard = (card:Pick<DeckCards, "id" | "cardId" | "role" >) => {
    ...
    }else
    {
      ...
      const newFormCards = currentCards.filter(
          c => !(String(c.cardId) === String(card.cardId) && String(c.role).trim() === String(card.role).trim())
      );
  ...
  const roleReplacer = (role) => {
    ...
    switch (role)
    {
      case "main": returnVal = "Principal"; break;
      case "side": returnVal = "Secondaire"; break;
    }
    return returnVal;
  }
  ...
  return (
    <>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            <ul className="space-y-1 max-h-64 overflow-y-auto">
              {cardsMap && cardsMap.map((c:any) => (
                  <li key={c.cardId+c.role} className="flex justify-between items-center border p-2 rounded">
                    <div>
                      <span className="mr-2">{c.name} x{c.qty}</span>
                      <Badge
                          variant="outline"
                      >
                        {roleReplacer(c.role)}
                    ...
                    <div>
                      <Button className="mr-2" size="sm" variant="secondary" onClick={() => {
                        if (c){
                          setCurrentCard(c);
                        }
                      ...
                      <Button size="sm" variant="destructive" onClick={() => removeCard({id: c.id, role: c.role, cardId: c.cardId})} type="button">
                        <Trash2 size={16} />
                      </Button>
                    </div>
                  </li>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Modification de la carte "{currentCard?.name ?? ''}"</DialogTitle>
          </DialogHeader>
          <div className="grid gap-4">
            <div className="grid gap-3">
              <Label htmlFor="cardQty">Quantité</Label>
              <Input id="cardQty" type="number" name="name" defaultValue={currentCard?.qty} onChange={(v)=>{
                let value = v.currentTarget.value;
                if (value){
                  setUpdatedCard((c) => (c ? { ...c, qty: parseInt(value) } : null));
                }
              }}/>
            </div>
            <div className="grid gap-3">
              ...
              <Select
                  ...
                  onValueChange={(v) => {
                    if (v){
                      setUpdatedCard((c) => (c ? ({ ...c, role: v } as DeckCards) : null));
                    }
          ...
          <DialogFooter>
            ...
            <Button onClick={()=> {
              ...
              if (
                ...
                updateCard({...updatedCard, cardId: currentCard?.cardId}, {qty: currentCard!.qty, role: currentCard!.role})
              }
