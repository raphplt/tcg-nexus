...
const buildHistoryId = () => `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
...
export default function ScanScreen() {
  ...
  const captureCard = async () => {
    ...
    try {
      ...
      if (!picture?.uri) {
        throw new Error("Capture impossible. Reessaie dans une zone bien eclairee.");
      }
      ...
      if (resolution.bestCard) {
        pushHistory({
          message: resolution.bestCard.name || "Carte identifiee automatiquement",
          status: "found",
          title: "Carte reconnue",
        });
      } else {
        pushHistory({
          message: "Aucune correspondance automatique. Lance une recherche manuelle.",
          status: "not-found",
  ...
  const ensureTargetCollection = async (): Promise<string> => {
    ...
    if (!user?.id) {
      throw new Error("Utilisateur non connecte. Impossible de creer une collection.");
    }
  ...
  const addCardToCollection = async () => {
    ...
    try {
      ...
      const item = await collectionService.addCardToCollection(collectionId, selectedCard.id);
  ...
  if (mode === "camera") {
    return (
      <View style={styles.cameraContainer}>
        ...
        <View style={[styles.cameraBottomBar, { paddingBottom: insets.bottom + 18 }]}>
          <Pressable
          ...
          <Text style={styles.captureHint}>Place la carte dans le cadre puis capture.</Text>
        </View>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <View style={styles.compareCard}>
          ...
          <View style={styles.compareRow}>
            ...
            <View style={styles.compareColumn}>
              ...
              {selectedCard?.image ? (
                <Image source={{ uri: selectedCard.image }} style={styles.scanPreview} />
              ) : (
                <View style={styles.placeholderBox}>
                  <Text style={styles.placeholderText}>Aucune carte correspondante</Text>
                </View>
              )}
            </View>
          </View>
          ...
          {searchedTerms.length > 0 ? (
            <Text style={styles.ocrMeta}>Recherches: {searchedTerms.join(" | ")}</Text>
          ) : null}
        </View>
          ...
          >
            {isSaving ? (
              ...
              <Text style={styles.addButtonText}>
                {selectedCard ? "Ajouter a la collection" : "Selectionne une carte"}
              </Text>
            )}
          </Pressable>
        </View>
        ...
        {inlineError ? <Text style={styles.inlineErrorPanel}>{inlineError}</Text> : null}
      </ScrollView>
    </KeyboardAvoidingView>
  );
}
