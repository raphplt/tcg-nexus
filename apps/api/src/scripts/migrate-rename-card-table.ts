...
async function renameColumnIfExists(
  ...
  await queryRunner.query(
    `ALTER TABLE public.${table} RENAME COLUMN ${from} TO ${to};`,
  );
}
...
async function main() {
  ...
  try {
    ...
    if (hasPokemonCard && !hasCard) {
      ...
      await queryRunner.query(
        "ALTER TABLE public.pokemon_card RENAME TO card;",
      );
      ...
      if (cardCount === 0 && pokemonCount > 0) {
        console.log(
          "card table empty. Dropping card and renaming pokemon_card -> card.",
        );
        ...
        console.log(
          "Both card and pokemon_card are empty. Dropping pokemon_card.",
        );
        await queryRunner.query("DROP TABLE public.pokemon_card CASCADE;");
      } else {
        throw new Error(
