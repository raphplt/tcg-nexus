...
import { ChallengeType, ChallengeActionType } from '../challenge/enums/challenge.enum';
...
async function runSeed() {
  ...
  if (count > 0) {
    ...
    console.log(`Successfully inserted ${defaultChallenges.length} challenges into the Database.`);
  }
  ...
  const activeDailies = defaultChallenges.filter(c => c.type === ChallengeType.DAILY).map(c => activeChallengeRepo.create({ challenge: c, expiresAt: tmrw }));
  const activeWeeklies = defaultChallenges.filter(c => c.type === ChallengeType.WEEKLY).map(c => activeChallengeRepo.create({ challenge: c, expiresAt: endOfWeek }));
