...
| MKT-01 | Line snapshots now use the locked listing price, matching the authoritative order total | Existing changed-price regression extended to assert line price | Resumption, attempt-level idempotency, expiry/provider races and UI still pending |
| MKT-02 | Refund and paid cancellation no longer imply physical restocking; only unpaid reservation cancellation releases stock, idempotently | 40 order unit tests; 25 marketplace/order PostgreSQL E2E tests; ADR-006 | Full partial refund, return, audit, provider reconciliation and UI workflow; this ticket is not complete |
...
| Q-011 | Checkout | P0 | API idempotency key is order-based; web session state is transient; checkout snapshots used cart price while totals used freshly locked price | Price mismatch corrected with regression assertion; idempotency/resumption still pending | MKT-01 |
...
No schema migration is introduced by these slices. Checkout line prices now agree with the locked price used in the total. Existing card placeholders
without productKind remain supported by web; normal items expose the existing
card/sealed discriminator. Sealed relations and condition fields are additive.
...
| Enabled workspace lint rules | Passed |
| API unit suite | 145 suites, 1,301 tests passed; process exits normally after lifecycle fixes |
| Web unit suite | 32 suites, 145 tests passed; collection tests rerun after final label updates |
| PostgreSQL collection E2E | 7 tests passed on a disposable database |
| PostgreSQL marketplace/order E2E | 25 tests passed on a disposable database |
...
The price-snapshot regression was rerun with the 40-test OrderService suite after
